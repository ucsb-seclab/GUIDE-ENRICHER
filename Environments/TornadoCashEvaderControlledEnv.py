...
class TornadoCashEvaderControlledEnv(gym.Env):

    def __init__(self, env_config: EnvContext, chain, tc, other):
    ...
    def step(self, action):
        ...
        if ACTION == 1:
            if self.agent_wallets[DEPOSIT_CALL_ADDR].get_balance() > 0 and \
                    ...
                    not TornadoCashEvaderControlledEnv.address_check(in_address=self.agent_challenge_table_addresses,
                                                                     picked_address=WITHDRAW_CALL_ADDR_OR_IN_ADDR) and \
                    TornadoCashEvaderControlledEnv.address_check(in_address=self.agent_challenge_table_addresses,
        ...
        self.state = np.array([
            ...
            self.chain.get_balance_of_addr('TC'),  # balance TC contract
            sum(self.agent_challenge_table),  # remaining of the challenge table
            1 if TornadoCashEvaderControlledEnv.address_check(in_address=self.agent_challenge_table_addresses,
                                                              picked_address=DEPOSIT_CALL_ADDR) else 0,
            # is DEPOSIT_CALL_ADDR is an address from challenge table
            1 if not TornadoCashEvaderControlledEnv.address_check(in_address=self.agent_challenge_table_addresses,
