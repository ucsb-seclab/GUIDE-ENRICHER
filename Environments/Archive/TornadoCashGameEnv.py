...
@DeprecationWarning
class TornadoCashGameEnv(gym.Env):
    ...
    def __init__(self, env_config: EnvContext):
        ...
        self.observation_space = gym.spaces.Box(low=0, high=10000, shape=(8,), dtype=np.float64)
    ...
    def step(self, action):
        ...
        if ACTION == 1:
            if self.agent_wallets[DEPOSIT_CALL_ADDR].get_balance() > 0 and \
                    ...
                    not TornadoCashGameEnv.address_check([0, 1], WITHDRAW_CALL_ADDR_OR_IN_ADDR) and \
                    TornadoCashGameEnv.address_check([0, 1], DEPOSIT_CALL_ADDR):
            ...
            reward = 0
        # CROWD FILTER
        # filterd_crowd_steps(num_of_crowd_steps=AGENT_WAITING_TIME)
        # CROWD FILTER
